...
import { subjectAdd } from '@/services/subject'
...
const SubjectEdit: FC = () => {
  ...
  const { subject, getSubject } = useModel('useSubject')
      ...
      {((id && subject) || !id) && (
        <ProForm<ISubject>
          ...
          onFinish={async values => {
            console.log(values)
            return
            // const cid = values.cid as any
            // const params = { ...values, cid: cid.length > 1 ? cid[1] : cid[0] }
        ...
        >
          ...
          />
          <ProForm.Group size={5}>
            <ProFormText width="lg" name="name" label="名称" placeholder="名称" rules={[{ required: true }]} required={false} />
